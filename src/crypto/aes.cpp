...
static inline Block calc_J0(const Block &H, byte *iv, const std::size_t iv_size)
{
    ...
    else
    {
        ...
        const auto s = 128 * ceil(len_iv / 128.0) - len_iv;
                                ...
                                const std::size_t key_size, const byte *iv, const std::size_t iv_size, byte *tag, const std::size_t tag_size)
{
    ...
    const Block H = calc_H(rkeys);
    const Block J0 = calc_J0(H, const_cast<byte *>(iv), iv_size);
    ...
    const Block S = ghash(H, ghash_in);
    const std::vector<byte> T = gctr(rkeys, J0, S.Data(), crypto::aes::BLOCK_BYTE_SIZE);
    ...
    unsigned char *crypted)
{
    ...
    const std::vector<unsigned char> C = gctr(rkeys, inc32(J0.Bits()), data, data_size);
...
{
    return data_size + crypto::aes::STATE_SIZE - (data_size % crypto::aes::STATE_SIZE);
}
...
namespace detail
{
    ...
    {
        // check data size
        if (!pads && (data_size % crypto::aes::STATE_SIZE != 0))
        {
        ...
        const unsigned long *padded_size)
    {
        ...
        {
            if (decrypted_size < (data_size - crypto::aes::STATE_SIZE))
            {
                return kErrorInvalidBufferSize;
            }
        }
    ...
    inline bool check_padding(const unsigned long padding, const unsigned char data[crypto::aes::STATE_SIZE])
    {
        if (padding > crypto::aes::STATE_SIZE)
        {
            return false;
        }
        ...
        {
            if (data[crypto::aes::STATE_SIZE - 1 - i] != padding)
            {
                return false;
            }
        }
    ...
    const bool pads)
{
    ...
    const unsigned long bc = data_size / crypto::aes::STATE_SIZE;
    for (unsigned long i = 0; i < bc; ++i)
    {
        encrypt_state(rkeys, data + (i * crypto::aes::STATE_SIZE), encrypted + (i * crypto::aes::STATE_SIZE));
    }

    if (pads)
    {
        const int rem = data_size % crypto::aes::STATE_SIZE;
        const char pad_v = crypto::aes::STATE_SIZE - rem;

        std::vector<unsigned char> ib(crypto::aes::STATE_SIZE, pad_v), ob(crypto::aes::STATE_SIZE);
        memcpy(&ib[0], data + data_size - rem, rem);

        encrypt_state(rkeys, &ib[0], &ob[0]);
        memcpy(encrypted + (data_size - rem), &ob[0], crypto::aes::STATE_SIZE);
    }
    ...
    unsigned long *padded_size)
{
    ...
    const unsigned long bc = data_size / crypto::aes::STATE_SIZE - 1;
    for (unsigned long i = 0; i < bc; ++i)
    {
        decrypt_state(rkeys, data + (i * crypto::aes::STATE_SIZE), decrypted + (i * crypto::aes::STATE_SIZE));
    }

    unsigned char last[crypto::aes::STATE_SIZE] = {};
    decrypt_state(rkeys, data + (bc * crypto::aes::STATE_SIZE), last);

    if (padded_size)
    {
        *padded_size = last[crypto::aes::STATE_SIZE - 1];
        const unsigned long cs = crypto::aes::STATE_SIZE - *padded_size;

        if (!detail::check_padding(*padded_size, last))
        {
            return kErrorInvalidKey;
        }
        else if (decrypted_size >= (bc * crypto::aes::STATE_SIZE) + cs)
        {
            memcpy(decrypted + (bc * crypto::aes::STATE_SIZE), last, cs);
        }
        else
        {
            return kErrorInvalidBufferSize;
    ...
    {
        memcpy(decrypted + (bc * crypto::aes::STATE_SIZE), last, sizeof(last));
    }
    ...
    const bool pads)
{
    ...
    unsigned char s[crypto::aes::STATE_SIZE] = {}; // encrypting data

    // calculate padding value
    const bool ge16 = (data_size >= crypto::aes::STATE_SIZE);
    const int rem = data_size % crypto::aes::STATE_SIZE;
    const unsigned char pad_v = crypto::aes::STATE_SIZE - rem;

    // encrypt 1st state
    if (ge16)
    {
        memcpy(s, data, crypto::aes::STATE_SIZE);
    }
    else
    {
        memset(s, pad_v, crypto::aes::STATE_SIZE);
    ...
    encrypt_state(rkeys, s, encrypted);

    // encrypt mid
    const unsigned long bc = data_size / crypto::aes::STATE_SIZE;
    for (unsigned long i = 1; i < bc; ++i)
    {
        const long offset = i * crypto::aes::STATE_SIZE;
        memcpy(s, data + offset, crypto::aes::STATE_SIZE);
        xor_data(s, encrypted + offset - crypto::aes::STATE_SIZE);

        encrypt_state(rkeys, s, encrypted + offset);
    }
    ...
    if (pads && ge16)
    {
        ...
        xor_data(&ib[0], encrypted + (bc - 1) * crypto::aes::STATE_SIZE);
    ...
    unsigned long *padded_size)
{
    ...
    // decrypt 1st state
    decrypt_state(rkeys, data, decrypted);
    if (iv)
    {
        xor_data(decrypted, *iv);
    }
    ...
    {
        const long offset = i * crypto::aes::STATE_SIZE;
        decrypt_state(rkeys, data + offset, decrypted + offset);
        xor_data(decrypted + offset, data + offset - crypto::aes::STATE_SIZE);
    }

    // decrypt last
    unsigned char last[crypto::aes::STATE_SIZE] = {};
    if (data_size > crypto::aes::STATE_SIZE)
    {
        decrypt_state(rkeys, data + (bc * crypto::aes::STATE_SIZE), last);
        xor_data(last, data + (bc * crypto::aes::STATE_SIZE - crypto::aes::STATE_SIZE));
    }
    else
    {
    ...
    const std::size_t tag_size)
{
    ...
    crypt_gcm(data, data_size, key, key_size, iv, iv_size, data);
    calc_gcm_tag(data, data_size, aadata, aadata_size, key, key_size, iv, iv_size, tag, tag_size);
    ...
    const std::size_t tag_size)
{
    ...
    calc_gcm_tag(C, C_size, aadata, aadata_size, key, key_size, iv, iv_size, tagd, 16);
    ...
    {
        crypt_gcm(C, C_size, key, key_size, iv, iv_size, C);
    ...
    const unsigned char (*nonce)[16])
{
    ...
    unsigned long pos = 0;
    unsigned long blkpos = crypto::aes::STATE_SIZE;
    unsigned char blk[crypto::aes::STATE_SIZE] = {};
    unsigned char counter[crypto::aes::STATE_SIZE] = {};
    memcpy(counter, nonce, 16);

    while (pos < data_size)
    {
        if (blkpos == crypto::aes::STATE_SIZE)
        {
            encrypt_state(rkeys, counter, blk);
