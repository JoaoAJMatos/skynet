...
crypto::CEcdsa::CEcdsa() {
    ...
    if (fill_random(this->randomize, sizeof(this->randomize)) != OK) {
        throw std::runtime_error("Could not generate randomness for ECDSA");
    }

    if (secp256k1_context_randomize(this->ctx, this->randomize) != 1) {
...
crypto::CEcdsa::~CEcdsa() {
    ...
    // Best practice to clear this from memory
    memset(this->randomize, 0, sizeof(this->randomize));
    memset(this->privkey, 0, sizeof(this->privkey));
}
 ...
 * @return ErrorCode - OK if successful, ERROR otherwise
 */
ErrorCode crypto::CEcdsa::GenerateKeyPair(ecdsa_keypair *keypair) {
    ...
    if (secp256k1_ec_pubkey_create(this->ctx, &pubkey, this->privkey) != 1) return ERROR;
 ...
 * @return ErrorCode - OK if successful, ERROR otherwise
 */
ErrorCode crypto::CEcdsa::Sign(byte *private_key, byte *hash, byte *signature) {
    secp256k1_ecdsa_signature sig;
    if (secp256k1_ecdsa_sign(this->ctx, &sig, hash, private_key, secp256k1_nonce_function_rfc6979, NULL) != 1) return ERROR;
    if (secp256k1_ecdsa_signature_serialize_compact(this->ctx, signature, &sig) != 1) return ERROR;
    return OK;
}
 ...
 * @param keypair
 */
void crypto::CEcdsa::PrintKeyPair(ecdsa_keypair *keypair) {
    printf("Private key: ");
